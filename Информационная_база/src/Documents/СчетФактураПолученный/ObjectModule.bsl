...
Процедура ВыполнитьОбработкуЗаполнения()
	...
	Если НЕ ДополнительныеСвойства.Свойство("ЭтоСинхронизация")
		...
		Если ВалидныеТипыДокументов.Найти(ТипЗнч(УдалитьДокументОснование)) <> Неопределено Тогда
			
			ДатаВходящегоДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УдалитьДокументОснование, "ДатаВходящегоДокумента");
			Если НЕ ЗначениеЗаполнено(ДатаВходящегоДокумента) Тогда
				
				ДатаВходящегоДокумента = УдалитьДокументОснование.Дата;
