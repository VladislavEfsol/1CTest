...
Процедура УстановитьВидимостьОтПользовательскихНастроек()
	...
	Если Объект.ПоложениеСклада = ПредопределенноеЗначение("Перечисление.ПоложениеРеквизитаНаФорме.ВТабличнойЧасти") Тогда
		...
		Элементы.ЗНМатериалыЯчейка.Видимость = ЗаказНарядВСостоянииВыполнения;
		Элементы.ЗапасыСтруктурнаяЕдиницаРезерв.Видимость = Истина;
		Элементы.ЗапасыЯчейка.Видимость = ЗаказНарядВСостоянииВыполнения;
	Иначе
		СкладВШапке = Истина;
	...
	Для Каждого ИмяТЧ из МассивИменТЧ Цикл
		...
		КонтекстноеМенюГруппаРаботаССтроками = ИмяТЧ + "КонтекстноеМенюГруппаРаботаССтроками";
			...
			Иначе
				...
				Элементы[СтруктурнаяЕдиницаРезерв].Видимость = Не РежимОстаткиИРезервы И РазрешитьСкладыВТабличныхЧастях;
