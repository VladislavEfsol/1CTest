...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбменЧерезОператора = ОбменЧерезОператора(Объект.СпособОбменаЭД);
...
Процедура ДобавитьСертификатПодписиВСоглашение(Отпечаток, СертификатДобавлен)
	...
	Для Каждого ЭП Из ЭлектронныеПодписиЭД Цикл
		Если ЭП.Отпечаток = Отпечаток Тогда
			НачатьТранзакцию();
			Попытка
				СоглашениеОбъект = ЭлектронноеВзаимодействиеСлужебный.ОбъектПоСсылкеДляИзменения(Объект.УдалитьНастройкаЭДО);
...
&НаСервере
Функция ОбменЧерезОператора(Знач СпособОбменаЭД)
	
	Если Не ЗначениеЗаполнено(СпособОбменаЭД) Тогда
		СпособОбменаЭД = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Ссылка, "УдалитьПрофильНастроекЭДО.СпособОбменаЭД");
	КонецЕсли;
	
	Если Не ОбменСКонтрагентамиСлужебный.ЭтоПрямойОбмен(СпособОбменаЭД) Тогда
		Возврат Истина
	Иначе
		Возврат Ложь;
	КонецЕсли;
