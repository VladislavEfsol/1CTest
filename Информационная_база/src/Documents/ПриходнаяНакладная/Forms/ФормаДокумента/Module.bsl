...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураНадписи = Новый Структура("ВалютаДокумента, ВалютаРасчетов, Курс, КурсНациональнаяВалюта, СуммаВключаетНДС, УчетВалютныхОпераций, ВидЦенКонтрагента, НалогообложениеНДС, СпециальныйНалоговыйРежим", Объект.ВалютаДокумента, ВалютаРасчетов, Объект.Курс, КурсНациональнаяВалюта, Объект.СуммаВключаетНДС, УчетВалютныхОпераций, Объект.ВидЦенКонтрагента, Объект.НалогообложениеНДС, Объект.СпециальныйНалоговыйРежим);
	...
	// ИнтеграцияМОТП
	ФОВестиУчетТабачнойПродукцииМОТП = ПолучитьФункциональнуюОпцию("ВестиУчетТабачнойПродукцииМОТП");
	Если ФОВестиУчетТабачнойПродукцииМОТП Тогда
		ПроверкаИПодборПродукцииМОТП.ПриСозданииНаСервере(ЭтотОбъект);
		Если Элементы.Найти("ВозобновитьПроверкуПродукцииМОТП") <> Неопределено Тогда
			Элементы.Переместить(Элементы.ВозобновитьПроверкуПродукцииМОТП, Элементы.Группа1);
		КонецЕсли;
		ЗаполнитьПризнакМарикруемойПродукции();
		ПроверкаИПодборПродукцииМОТП.ЗаполнитьКешШтрихкодовУпаковок(ЭтотОбъект);
		ИнтеграцияМОТПУНФ.ПрименитьКешШтрихкодовУпаковок(ЭтотОбъект);
	КонецЕсли;
	ЗапасыКешТекущейСтроки = Новый Структура;
	// Конец ИнтеграцияМОТП
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ФОВестиУчетТабачнойПродукцииМОТП Тогда
		...
		Если Элементы.Найти("ВозобновитьПроверкуПродукцииМОТП") <> Неопределено
			И Элементы.ВозобновитьПроверкуПродукцииМОТП.Родитель <> Элементы.Группа1 Тогда
			Элементы.Переместить(Элементы.ВозобновитьПроверкуПродукцииМОТП, Элементы.Группа1);
		КонецЕсли;
	КонецЕсли;
	// Конец ИнтеграцияМОТП
	...
	Если ТекущийОбъект.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика Тогда
...
Процедура КонтрагентПриИзмененииФрагмент(ДоговорПередИзменением, СтруктураДанные)
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.Договор) 
		...
		Объект.ВидЦенКонтрагента	= СтруктураДанные.ВидЦенКонтрагента;
		СтруктураНадписи 			= 
			Новый Структура("ВалютаДокумента, ВалютаРасчетов, Курс, КурсНациональнаяВалюта, СуммаВключаетНДС, УчетВалютныхОпераций, ВидЦенКонтрагента, НалогообложениеНДС, СпециальныйНалоговыйРежим", 
...
Процедура ЗапасыПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если ФОВестиУчетТабачнойПродукцииМОТП Тогда
		...
		ЗаполнитьПризнакМаркируемойПродукцииВСтроке(СтрокаЗапасы);
...
Процедура ЗапасыПриАктивизацииСтроки(Элемент)
	...
	Если ФОВестиУчетТабачнойПродукцииМОТП Тогда
		...
		Если СтрокаТабличнойЧасти <> Неопределено Тогда
			ЗапасыКешТекущейСтроки.Вставить("Номенклатура",					СтрокаТабличнойЧасти.Номенклатура);
			ЗапасыКешТекущейСтроки.Вставить("Характеристика",				СтрокаТабличнойЧасти.Характеристика);
			ЗапасыКешТекущейСтроки.Вставить("Серия",						СтрокаТабличнойЧасти.Партия);
			ЗапасыКешТекущейСтроки.Вставить("МаркируемаяПродукцияГосИС",	СтрокаТабличнойЧасти.МаркируемаяПродукцияГосИС);
		КонецЕсли;
	КонецЕсли;
	// Конец МОТП
...
// ГосИС
&НаСервере
Процедура ЗаполнитьПризнакМарикруемойПродукции()
...
Процедура ПрименитьКешПоСтрокеМОТП()
	...
	ЗапасыТекущаяСтрока = Новый Структура;
	ЗапасыТекущаяСтрока.Вставить("Номенклатура",				ТекущаяСтрока.Номенклатура);
	ЗапасыТекущаяСтрока.Вставить("Характеристика",				ТекущаяСтрока.Характеристика);
	ЗапасыТекущаяСтрока.Вставить("Серия",						ТекущаяСтрока.Партия);
	ЗапасыТекущаяСтрока.Вставить("МаркируемаяПродукцияГосИС",	ТекущаяСтрока.МаркируемаяПродукцияГосИС);
	ЗапасыТекущаяСтрока.Вставить("СтатусПроверкиГосИС",			ТекущаяСтрока.СтатусПроверкиГосИС);
	
	НужноПересчитатьКеш = ИнтеграцияМОТПУНФКлиент.ПрименитьКешПоСтроке(
		ЭтотОбъект,
		ЗапасыТекущаяСтрока,
		ЗапасыКешТекущейСтроки);
...
Процедура ПересчитатьКешМаркируемойПродукции()
	...
	ИнтеграцияМОТПУНФ.ПрименитьКешШтрихкодовУпаковок(
		ЭтотОбъект,
		Истина);
...
Процедура ЗаполнитьСтавкуНДСПоОрганизациияНалогообложениеНДС(ЭтоОткрытие = Ложь)
	...
	ИначеЕсли  Объект.ВидОперации = Перечисления.ВидыОперацийПриходнаяНакладная.ПоступлениеОтПоставщика Тогда
