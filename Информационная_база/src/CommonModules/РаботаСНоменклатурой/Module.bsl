...
Процедура ЗагрузкаКатегорий(Знач ПараметрыЗагрузки, ДолжноБытьСоздано, Знач АдресРезультата, Знач АдресДополнительногоРезультата)
	...
	ИначеЕсли ПараметрыЗагрузки.РежимЗагрузкиКатегорий = "ПоОтдельности" Тогда	
		...
		ЗагрузитьКатегорииПоОтдельности(ДанныеКатегорий, ПараметрыЗагрузки, НовыеЭлементы);
...
Процедура ЗагрузитьКатегорииПоОтдельности(ДанныеКатегорий, ПараметрыЗагрузки, НовыеЭлементы)
...
Процедура ЗаполнитьВстроенныеРеквизитыФормыНоменклатурыДляРежимаСопоставление(Форма, ГруппаРодитель = Неопределено)
...
Процедура СформироватьКарточкуНоменклатуры(
	...
	ЗаполнитьПлатынеРеквизиты(ДанныеНоменклатуры, ПлатныеРеквизиты, КарточкаКуплена);
...
Процедура ЗаполнитьПлатынеРеквизиты(ДанныеНоменклатуры, ПлатныеРеквизиты, КарточкаКуплена)
...
Процедура УстановитьПараметрыДинамическогоСписка(ПараметрыЗапроса)
	...
	ПараметрыЗапроса.УстановитьЗначениеПараметра("СтрокаСопоставить",     НСтр("ru = 'Сопоставить'"));
...
Процедура ЗаписатьИнформациюОбОшибкеВЖурналРегистрации(ИмяСобытия, ИнформацияОбИсключении = Неопределено)
...
Процедура НайтиНоменклатуруПоШрихкодуИЗаписатьРезультат(ДанныеПоШтрихкодам, Отказ)
	...
	Для каждого Номенклатура Из НоменклатураПоШтрихкодам Цикл
		Для каждого Штрихкод Из Номенклатура.Штрихкоды Цикл
			...
			СтрокиПоискаШтрихкоды = ДанныеПоШтрихкодам.НайтиСтроки(Новый Структура("Штрихкод", Штрихкод));
			...
			Для каждого ЭлементСтрокиТаблицыПоиска Из СтрокиПоискаШтрихкоды Цикл
				...
				НоваяСтрока.ИдентификаторНоменклатурыСервиса = Номенклатура.Идентификатор;
				НоваяСтрока.ПредставлениеНоменклатурыСервиса = Номенклатура.Наименование;
	...
	Попытка
		...
		Для каждого ЭлементКоллекции Из НоменклатураИХарактеристики Цикл
			...
			Если СтрокиПоСсылкам.Количество() > 0 Тогда
				...
				Запись.ИдентификаторНоменклатурыСервиса = СтрокиПоСсылкам[0].ИдентификаторНоменклатурыСервиса;
				Запись.ПредставлениеНоменклатурыСервиса = СтрокиПоСсылкам[0].ПредставлениеНоменклатурыСервиса;
