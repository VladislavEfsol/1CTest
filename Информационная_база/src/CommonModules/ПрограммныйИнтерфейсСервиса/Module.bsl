...
Функция АбонентЭтогоПриложения(Знач Пользователь = Неопределено) Экспорт
	...
	ДанныеЗапроса.Вставить("auth", ДанныеАвторизации(Неопределено, Пользователь));
...
Функция ДанныеАвторизации(КодАбонента = Неопределено, Знач Пользователь = Неопределено)  
