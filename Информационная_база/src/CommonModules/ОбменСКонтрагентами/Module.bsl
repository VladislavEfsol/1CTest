...
Процедура ЗаполнитьКешиОператоровЭДОИФорматов(Параметры) Экспорт
	...
	Попытка
		...
		// Если есть запись хотя бы об одном поддерживаемом формате, то обновление не требуется.
		Для каждого Запись Из НаборЗаписей Цикл
			Если Запись.ПоддерживаетсяКонфигурацией Тогда
				ОбновитьФорматы = Ложь;
			КонецЕсли;
		КонецЦикла;
		...
		Если ОбновитьФорматы Тогда
			...
			Для Каждого ФорматЭДО Из ТаблицаФорматовЭДО Цикл
				...
				НовыйФормат.ПространстваИмен                         = ТекущийФормат.ПространстваИмен;
		...
		НаборЗаписей.Прочитать();
		ТаблицаОператорыЭДО = НаборЗаписей.Выгрузить();
...
#Область Справочники_ЭДПрисоединенныеФайлы_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Документы.СчетНаОплатуПоставщика.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	НоваяСтрока.Порядок = "До";

	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Документы._ДемоОперация.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	НоваяСтрока.Порядок = "До";

	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Документы.ЗаказПокупателя.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	НоваяСтрока.Порядок = "До";

	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Документы.АктОРасхожденияхПоПриемке.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	НоваяСтрока.Порядок = "До";

	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Документы.АктОРасхожденияхПоОтгрузке.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	НоваяСтрока.Порядок = "До";
