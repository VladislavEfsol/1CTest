...
Процедура ВыполнитьОбмен(УзелОбмена, РежимЗапускаОбмена, ВыгружатьТолькоИзменения = Истина, ПараметрыСоставОбмена=Неопределено) Экспорт
	...
	Параметры.Вставить("ТаблицаИнформации", ТаблицаИнформации);
...
Функция ПодготовитьДанныеXDTOКлассификаторИКаталог(Параметры, ПакетCML)
	...
	Если НЕ Параметры.ИндексФайлаОбмена > 1 Тогда
		...
		Если Параметры.ОбменЗаписьНаУслуги И Параметры.РежимВыгрузкиУслуг Тогда
			ДобавитьРесурсыКлассификатораXDTO(КлассификаторXDTO, Параметры.Услуги, ПакетCML, Параметры);
		КонецЕсли;
	...
	Если ОбъектXDTOСодержитСвойство(КонтрагентXDTO, "ЮридическийАдрес") Тогда
		...
		Если НЕ АдресXDTO = Неопределено 
			...
			Если НЕ АдресНайден Тогда
				...
				Если НЕ Справочники.ВидыКонтактнойИнформации.ЮрАдресКонтрагента.РазрешитьВводНесколькихЗначений Тогда
					ОтборКИ = Новый Структура("Вид,Тип", ВидКИ, ТипКИ);
					НайденныеСтрокиКИ = КонтрагентОбъект.КонтактнаяИнформация.НайтиСтроки(ОтборКИ);
					Если НайденныеСтрокиКИ.Количество()<>0 Тогда
						КонтрагентОбъект.КонтактнаяИнформация.Удалить(НайденныеСтрокиКИ[0]);
...
Процедура ПодготовитьДанныеДляВыгрузки(Параметры)
	...
	Иначе
		...
		Если Параметры.ОбменЗаписьНаУслуги И Параметры.РежимВыгрузкиУслуг Тогда
			ИзменитьОтборКомпоновщика(КомпоновщикНастроек,,Параметры.Услуги.ВыгрузитьКолонку("Услуга"));
		Иначе
			ИзменитьОтборКомпоновщика(КомпоновщикНастроек, Параметры.СтрокаТаблицыКаталогов.Группы);
		КонецЕсли;
	...
	Если Параметры.ОбменЗаписьНаУслуги Тогда
		...
		Запрос.УстановитьПараметр("Ресурсы", Параметры.Мастера);
	КонецЕсли;
	...
	Если Параметры.ОбменЗаписьНаУслуги И Параметры.РежимВыгрузкиУслуг Тогда
		Параметры.Вставить("ТаблицаГрафикЗанятостиРесурсов", МассивРезультатовЗапроса[54]);
	КонецЕсли;
...
Процедура ДобавитьЗапросыВПакет(ТекстЗапроса, Параметры)
	...
	Если Параметры.ОбменЗаписьНаУслуги И Параметры.РежимВыгрузкиУслуг Тогда
		...
		|ИЗ
		|	РегистрСведений.ГрафикиРаботыРесурсов.СрезПоследних(&ТекущаяДата, РесурсПредприятия В (&Ресурсы)) КАК ГрафикиРаботыРесурсов
		...
		|	И ГрафикиРаботыРесурсов.РесурсПредприятия В(&Ресурсы)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	втГрафикПлан.НомерДняЦикла КАК НомерДняЦикла,
		...
		|		ПО втГрафикПлан.Ресурс = ОтклоненияОтГрафиковРаботыРесурсов.РесурсПредприятия
		|			И втГрафикПлан.Год = ОтклоненияОтГрафиковРаботыРесурсов.Год
		|			И (ОтклоненияОтГрафиковРаботыРесурсов.День = втГрафикПлан.ДЕНЬ)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	втОтклонения.НомерДняЦикла КАК НомерДняЦикла,
		...
		|			И (РасписаниеЗагрузкиРесурсов.Старт = втГрафикСОтклонениями.ВремяНачалаИнтервала)
		|ГДЕ
		...
		|	И КлючевыеРесурсы.Ссылка В(&Ресурсы)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Процедура ИзменитьОтборКомпоновщика(КомпоновщикНастроек, СписокГруппКаталога = Неопределено, МассивИзмененийНоменклатуры = Неопределено) Экспорт
	...
	ОтборПоУслугам    = "ПрограммныйОтборПоУслугам";
	...
	// Добавляем отбор по услугам для записи на услугу.
	//Если МассивИзмененийНоменклатуры <> Неопределено Тогда
	//	
	//	СписокУслугОтбора = Новый СписокЗначений;
	//	СписокУслугОтбора.ЗагрузитьЗначения(МассивИзмененийНоменклатуры);
	//	
	//	НовыйЭлемент = Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	//	НовыйЭлемент.ИдентификаторПользовательскойНастройки = ОтборПоУслугам;
	//	НовыйЭлемент.ЛевоеЗначение 	=  Новый ПолеКомпоновкиДанных("Номенклатура");
	//	НовыйЭлемент.ВидСравнения 	= ВидСравненияКомпоновкиДанных.ВСписке;
	//	НовыйЭлемент.ПравоеЗначение = СписокУслугОтбора;
	//	НовыйЭлемент.Использование 	= Истина;
	//	
	//КонецЕсли;	
...
Функция ПараметрыДляУслуг(Параметры)
	...
	//ПараметрыДляУслуг.Вставить("МассивИзмененийНоменклатуры", ПараметрыДляУслуг.Услуги.ВыгрузитьКолонку("Услуга"));
...
Процедура ДобавитьНастройкиУзлаВПараметры(УзелОбмена, Параметры)
	...
	Запрос = Новый Запрос(
		...
		|	ОбменУправлениеНебольшойФирмойСайт.Ресурсы.(
		|		Ресурс КАК Ресурс,
		|		Ресурс.ПометкаУдаления КАК ПометкаУдаления,
		|		Ресурс.Наименование КАК Наименование,
		|		Ресурс.Представление КАК Представление,
		|		Ресурс.КратностьПланирования КАК КратностьПланирования,
		|		Ресурс.ЗначениеРесурса КАК Сотрудник
		|	) КАК Ресурсы,
		|	ОбменУправлениеНебольшойФирмойСайт.Услуги.(
		|		Услуга КАК Услуга,
		|		Услуга.ПометкаУдаления КАК ПометкаУдаления,
		|		Длительность КАК Длительность,
		|		Услуга.Представление КАК УслугаПредставление
		|	) КАК Услуги,
		...
		|	ОбменУправлениеНебольшойФирмойСайт.СохраненнаяТаблицаУслуг КАК СохраненнаяТаблицаУслуг		
		|ИЗ
	...
	Если УзелОбмена.ОбменЗаписьНаУслуги Тогда
		...
		Если Параметры.ВыгружатьТолькоИзменения Тогда
			Запрос = Новый Запрос;
			Запрос.Текст = "ВЫБРАТЬ
			|	КлючевыеРесурсыИзменения.Ссылка КАК Ресурс
			|ИЗ
			|	Справочник.КлючевыеРесурсы.Изменения КАК КлючевыеРесурсыИзменения
			|ГДЕ
			|	КлючевыеРесурсыИзменения.Узел = &Узел";
			Запрос.УстановитьПараметр("Узел", УзелОбмена);
			Параметры.Вставить("Мастера", Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ресурс"));
		Иначе
			Параметры.Вставить("Мастера", Параметры.Ресурсы.ВыгрузитьКолонку("Ресурс"));
		КонецЕсли;
...
Процедура ДобавитьРесурсыКлассификатораXDTO(КлассификаторXDTO, Услуги, ПакетCML, Параметры) Экспорт
	
	//список услуг
	СохраненнаяТаблицаУслуг = Параметры.СохраненнаяТаблицаУслуг.Получить();
	ВсеРесурсы = ВсеРесурсы();
	
	Если ТипЗнч(СохраненнаяТаблицаУслуг) = Тип("ТаблицаЗначений") Тогда
		
		ТаблицаУслугРесурсов = СохраненнаяТаблицаУслуг.СкопироватьКолонки("Услуга, Мастера, Длительность");
		ТаблицаУслугРесурсов.Колонки.Добавить("Ресурс", Новый ОписаниеТипов("СправочникСсылка.КлючевыеРесурсы"));
		Для Каждого СтрокаСохраненнойТаблицыУслуг Из СохраненнаяТаблицаУслуг Цикл
			Если СтрокаСохраненнойТаблицыУслуг.Мастера.Количество() = 1 
				И НЕ ЗначениеЗаполнено(СтрокаСохраненнойТаблицыУслуг.Мастера[0].Значение) Тогда
			
				Для каждого стрРесурс Из ВсеРесурсы Цикл
					НоваяСтрока = ТаблицаУслугРесурсов.Добавить();
					НоваяСтрока.Услуга = СтрокаСохраненнойТаблицыУслуг.Услуга;
					НоваяСтрока.Длительность = СтрокаСохраненнойТаблицыУслуг.Длительность;
					НоваяСтрока.Ресурс = стрРесурс.Ресурс;
				КонецЦикла;
				
			Иначе
				
				Для каждого стрРесурс Из СтрокаСохраненнойТаблицыУслуг.Мастера Цикл
					НоваяСтрока = ТаблицаУслугРесурсов.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаСохраненнойТаблицыУслуг);
					НоваяСтрока.Ресурс = стрРесурс.Значение;
				КонецЦикла;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТУслуги.Ресурс КАК Ресурс,
	|	ВТУслуги.Ресурс.ПометкаУдаления КАК РесурсПометкаУдаления,
	|	ВТУслуги.Услуга КАК Услуга,
	|	ВТУслуги.Длительность КАК Длительность,
	|	ВТУслуги.Ресурс.Представление КАК РесурсПредставление,
	|	ВТУслуги.Ресурс.КратностьПланирования КАК КратностьПланирования,
	|	ВТУслуги.Ресурс.ЗначениеРесурса КАК Сотрудник,
	|	ВТУслуги.Услуга.Представление КАК УслугаПредставление
	|ИЗ
	|	ВТУслуги КАК ВТУслуги
	|ИТОГИ ПО
	|	Ресурс";
	Запрос.УстановитьПараметр("Услуги", ТаблицаУслугРесурсов);
	Результат = Запрос.Выполнить();
	
	ВыборкаРесурс = Результат.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	Пока ВыборкаРесурс.Следующий() Цикл
