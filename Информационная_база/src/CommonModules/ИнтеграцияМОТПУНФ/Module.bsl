...
Процедура ОбработатьШтрихкоды(Форма, ДанныеДляОбработки, КэшированныеЗначения) Экспорт
	...
	СтруктураДанные = Новый Структура("МассивШтрихкодов, ВидОперации, Организация,ВидЦен, Дата, ВалютаДокумента,
									  |СуммаВключаетНДС, ВидСкидкиНаценки, НалогооблажениеНДС, ПараметрыВыбора,
									  |ПроцентСкидкиПоДисконтнойКарте, ДисконтнаяКарта");
	
	ЗаполнитьЗначенияСвойств(СтруктураДанные, Объект);
	
	СтруктураДанные.Вставить("МассивШтрихкодов", МассивШтрихкодов);
	СтруктураДанные.Вставить("ПараметрыВыбора", Форма.Элементы.ЗапасыНоменклатура.ПараметрыВыбора);
	...
	Для каждого ТекШтрихкод Из СтруктураДанные.МассивШтрихкодов Цикл
		...
		Иначе
			...
			Если МассивСтрокТЧ.Количество() = 0 Тогда
				...
				НоваяСтрока.ЯчейкаДоступна = ЯчейкаДоступна(Форма, НоваяСтрока.СтруктурнаяЕдиница);
				
				Если ЭтоПродажа(СтруктураДанные.ВидОперации) И ДанныеШтрихкода.СтруктураДанныеНоменклатуры.ЭтоНабор Тогда
					// Наборы
					ЗаменитьСтрокуЗапасовДаннымиНабора(Форма, НоваяСтрока, ДанныеШтрихкода.СтруктураДанныеНоменклатуры);
					// Конец Наборы
					// АвтоматическиеСкидки.
					СброситьФлагСкидкиРассчитаны(Форма, "РассчитатьСуммуВСтрокеТабличнойЧасти", "Сумма");
					// Конец АвтоматическиеСкидки
				Иначе
					РассчитатьСуммуВСтрокеТабличнойЧасти(НоваяСтрока, Новый Структура("СуммаВключаетНДС", Объект.СуммаВключаетНДС));
					Форма.Элементы.Запасы.ТекущаяСтрока = НоваяСтрока.ПолучитьИдентификатор();
				КонецЕсли; 
	...
	// Необходимо реализовать обработку неизвестных штрихкодов. Как? - не понятно.
...
Процедура ПриОпределенииПараметровИнтеграцииФормыПроверкиИПодбора(Форма, ПараметрыИнтеграции) Экспорт
	...
	ПараметрыИнтеграции.ИспользоватьБезТабачнойПродукции = ЭтоРасходнаяНакладная;
...
#Область ИнтеграцияМОТППереопределяемый
...
Процедура ОтразитьРезультатыПроверкиИПодбораВДокументеПоступления(ПараметрыОкончанияСканирования)
	...
	Если Не ПараметрыОкончанияСканирования.СоздаватьАктОРасхождениях Тогда
		...
		Для Каждого СтрокаПодобраннойПродукции Из ПровереннаяИПодобраннаяПродукция Цикл
...
Функция СброситьФлагСкидкиРассчитаны(Форма, Действие, КолонокаТЧ = "")
	
	ТребуетсяПересчет = Ложь;
	Если Форма.Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРасходнаяНакладная.ПродажаПокупателю") Тогда
		Если Форма.ИспользоватьАвтоматическиеСкидки И Форма.Объект.Запасы.Количество() > 0 И (Форма.Объект.СкидкиРассчитаны ИЛИ Форма.УстановленСерыйЦвет) Тогда
			ТребуетсяПересчет = СкидкиНаценкиСервер.СброситьФлагСкидкиРассчитаны(Форма, Действие, КолонокаТЧ);
		КонецЕсли;
	КонецЕсли;
...
Функция ЯчейкаДоступна(Форма, СтруктурнаяЕдиница)
	
	Если НЕ Форма.КэшЗначений.УчетПоЯчейкам Тогда
		Возврат Ложь;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(СтруктурнаяЕдиница) ИЛИ ТипЗнч(СтруктурнаяЕдиница)<>Тип("СправочникСсылка.СтруктурныеЕдиницы") Тогда
		Возврат Ложь;
	КонецЕсли; 
	Если НЕ Форма.КэшЗначений.Свойство("ДоступностьЯчеек") Тогда
		Возврат Ложь;
	КонецЕсли; 
	
	Результат = Форма.КэшЗначений.ДоступностьЯчеек.Получить(СтруктурнаяЕдиница);
	Если Результат = Неопределено Тогда
		МассивСтруктурныхЕдиниц = Новый Массив;
		МассивСтруктурныхЕдиниц.Добавить(СтруктурнаяЕдиница);
		ДополнитьДоступностьЯчеек(МассивСтруктурныхЕдиниц, Форма.КэшЗначений.ДоступностьЯчеек);
		Результат = Форма.КэшЗначений.ДоступностьЯчеек.Получить(СтруктурнаяЕдиница);
		Если Результат=Неопределено Тогда
			Возврат Ложь;
		КонецЕсли; 
