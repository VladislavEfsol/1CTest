...
Процедура ЗаписатьИзмененияНаСервере(Отказ)
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			ИначеЕсли Выборка.Профиль.ИдентификаторПоставляемыхДанных = УправлениеДоступомПереопределяемый.ПрофильРедактированиеЦенДокументов().УИД Тогда
				...
				Отбор = Новый Структура("ГруппаДоступа, ВидДоступа", ПрофильПродажи, Справочники.ВидыЦен.ПустаяСсылка());
