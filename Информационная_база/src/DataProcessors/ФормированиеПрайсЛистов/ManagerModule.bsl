...
Процедура ПолучитьТаблицуЦен(НастройкиПрайсЛиста, ТаблицаЦен)
	...
	|	,РСЦены.ВидЦен.ВалютаЦены КАК ВалютаЦены
	...
	|Индексировать по ВалютаЦены
	|
	|;Выбрать
	|	РСКурсыВалют.Валюта КАК Валюта
	|	,РСКурсыВалют.Курс КАК Курс
	|	,РСКурсыВалют.Кратность КАК Кратность
	|Поместить ВременнаяТаблицаКурсыВалют
	|Из РегистрСведений.КурсыВалют.СрезПоследних(&ДатаРасчетаКурсаВалюты) КАК РСКурсыВалют
	|Индексировать по Валюта
	|
	|;Выбрать
	|	РСЦены.ВидЦен КАК ВидЦен
	|	,РСЦены.НоменклатураСсылка КАК НоменклатураСсылка
	|	,РСЦены.Характеристика КАК Характеристика
	|	,РСЦены.ВидЦен.ПорядокОкругления КАК ПорядокОкругления
	|	,РСЦены.ВидЦен.ОкруглятьВБольшуюСторону КАК ОкруглятьВБольшуюСторону
	|	,Выбор Когда РСЦены.ВалютаЦены = &Валюта Тогда РСЦены.Цена 
	...
	|	,РСЦены.ЕдиницаИзмерения КАК ЕдиницаИзмерения
	|Из
	|	ВременнаяТаблицаЦены КАК РСЦены
	|	Левое соединение ВременнаяТаблицаКурсыВалют КАК КурсВалютыВидаЦен
	|		По (РСЦены.ВалютаЦены = КурсВалютыВидаЦен.Валюта)
	|";
