...
Процедура УстановитьCryptoProCSPПослеПолученияДистрибутива(ДлительнаяОперация, ВходящийКонтекст) Экспорт
	...
	Если ДлительнаяОперация.Статус = "Выполнено" Тогда
		...
		ВходящийКонтекст.Вставить("НомерДистрибутива", "");
		...
		Если Результат.Свойство("СерийныйНомер") Тогда
			ВходящийКонтекст.Вставить("СерийныйНомер", Результат.СерийныйНомер);
		КонецЕсли;
		
		Оповещение = Новый ОписаниеОповещения(
			"УстановитьCryptoProCSPПослеПодключенияРасширенияРаботыСФайлами", ЭтотОбъект, ВходящийКонтекст);
...
&НаКлиенте
Процедура УстановитьCryptoProCSPПослеПодключенияРасширенияРаботыСФайлами(Подключено, ВходящийКонтекст) Экспорт
	
	Если Подключено Тогда
		НачатьПолучениеКаталогаВременныхФайлов(ВходящийКонтекст.ПослеСозданияВременногоКаталога)
	Иначе
		РезультатВыполнения = Новый Структура("Выполнено", Ложь);
...
&НаКлиенте
Процедура УстановитьCryptoProCSPПослеСозданияВременногоКаталога(ИмяКаталогаВременныхФайлов, ВходящийКонтекст) Экспорт
		...
		ПолучитьИзВременногоХранилища(ВходящийКонтекст.Дистрибутив).Записать(ИмяФайла);
		...
		ТекстовыйФайл = Новый ЗаписьТекста(ВходящийКонтекст.ВременныйКаталог + "Setup.bat",КодировкаТекста.OEM);
...
&НаКлиенте
Процедура УстановитьViPNetCSPПослеСозданияВременногоКаталога(ИмяКаталогаВременныхФайлов, ВходящийКонтекст) Экспорт
	
		ВходящийКонтекст.Вставить("ВременныйКаталог", ИмяКаталогаВременныхФайлов);
		
		ИмяФайла = ВходящийКонтекст.ВременныйКаталог + "Setup.exe";
		Поток = Новый ПотокВПамяти;
		ВходящийКонтекст.Дистрибутив.Записать(Поток);
		
		ИмяКаталогаВременныхФайловИзвлечь = ИмяКаталогаВременныхФайлов + СтрЗаменить(Строка(Новый УникальныйИдентификатор),"-", "") + "\";
		ЧтениеZip = Новый ЧтениеZipФайла(Поток);
		ЧтениеZip.ИзвлечьВсе(ИмяКаталогаВременныхФайловИзвлечь);
		...
		ТекстовыйФайл = Новый ЗаписьТекста(ИмяКаталогаВременныхФайловИзвлечь + "cspreg.txt",КодировкаТекста.OEM);
		ТекстовыйФайл.Записать(ТекстСкрипта);
		ТекстовыйФайл.Закрыть();
		...
		|
		|""!setup.exe!""!args!";
		ТекстСкрипта = СтрЗаменить(ТекстСкрипта, "!setup.exe!", ИмяКаталогаВременныхФайловИзвлечь + ЧтениеZip.Элементы[0].Имя);
		ТекстСкрипта = СтрЗаменить(ТекстСкрипта, "!args!", "");
		
		ТекстовыйФайл = Новый ЗаписьТекста(ИмяКаталогаВременныхФайловИзвлечь + "Setup.bat",КодировкаТекста.OEM);
		ТекстовыйФайл.Записать(ТекстСкрипта);
		ТекстовыйФайл.Закрыть();
		
		ЗапуститьПриложениеНаКлиенте(
			ИмяКаталогаВременныхФайловИзвлечь + "Setup.bat", ИмяКаталогаВременныхФайловИзвлечь, Истина, Ложь, ВходящийКонтекст);
КонецПроцедуры
