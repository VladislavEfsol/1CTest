...
Процедура ЗаполнитьДеревоУслуг()
	...
	ДеревоУслуг = Пакет[0].Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией);
	НетРесурсов = Пакет[1].Пустой();
	НетГрафиков = Пакет[2].Пустой();
	
	Если НетРесурсов ИЛИ НетГрафиков ИЛИ ДеревоУслуг.Строки.Количество() = 0 Тогда
		Элементы.ГруппаУслуги.Видимость = Ложь;
		Элементы.НастройкиУслуги.Видимость = Истина;
	КонецЕсли;
	
	ЗначениеВРеквизитФормы(ДеревоУслуг,"ОтборУслуги");
...
&НаКлиенте
Процедура ОтборУслугиФлагПриИзменении(Элемент)
	
	ПометкаПриИзменении("ОтборУслуги");
...
Функция ПараметрыСозданияСайта(ПараметрыВызоваСервера = Неопределено)
	...
	Если ЗаписьНаУслуги  И (АдресТаблицыУслуг = "") Тогда
		АдресТаблицыУслуг = ПоместитьВоВременноеХранилище(СписокВыбранныхУслуг("ОтборУслуги"), Новый УникальныйИдентификатор);
		ПараметрыВызоваСервера.Вставить("СписокУслуг", АдресТаблицыУслуг);
	Иначе
		ПараметрыВызоваСервера.Вставить("СписокУслуг", АдресТаблицыУслуг);
	КонецЕсли;
...
Функция СоздатьСайтВФонеПродолжить(ПараметрыСозданияСайта)
	...
	Если ЗаписьНаУслуги Тогда
		ХранилищеСистемныхНастроек.Сохранить("СписокУслугСозданияСайта",,СписокВыбранныхУслуг("ОтборУслуги"));
	КонецЕсли;
...
&НаСервере
Функция СписокВыбранныхУслуг(ИмяДереваЗначений)

	// Таблица каталогов.
	СписокУслуг = Новый СписокЗначений;
	УслугиВДереве = ЭтотОбъект[""+ИмяДереваЗначений].ПолучитьЭлементы();
	Если УслугиВДереве.Количество()=0 Тогда
		//пустой список услуг
		Возврат СписокУслуг;
	Иначе
		НайтиВыделенныеСтроки(УслугиВДереве, СписокУслуг);
	КонецЕсли;
...
Процедура ОткрытьСозданныйСайт(ДанныеСозданияСайтаСтруктура)
	...
	Если ЗаписьНаУслуги Тогда
	    ПерейтиПоНавигационнойСсылке(СтрЗаменить(ДанныеСозданияСайтаСтруктура.СсылкаАдминЗоны, "/adminzone/","/adminzone/content/appointments/"));
	Иначе
...
&НаКлиенте
Процедура УслугиУстановитьФлажки(Команда)
	УстановитьПометку(Истина, "ОтборУслуги");
КонецПроцедуры

&НаКлиенте
Процедура УслугиСнятьФлажки(Команда)
	УстановитьПометку(Ложь, "ОтборУслуги");
КонецПроцедуры
...
&НаКлиенте
Процедура СвернутьУслуги(Команда)
	
	Для каждого строкаДерева Из ОтборУслуги.ПолучитьЭлементы() Цикл 
		Элементы.ОтборУслуги.Свернуть(строкаДерева.ПолучитьИдентификатор()); 
	КонецЦикла; 
	Элементы.Развернуть.Видимость = Истина;
	Элементы.Свернуть.Видимость = Ложь;
	
	ЭтотОбъект.ТекущийЭлемент = Элементы.ОтборУслуги;
	
КонецПроцедуры

&НаКлиенте
Процедура РазвернутьУслуги(Команда)
	
	УправлениеНебольшойФирмойКлиент.РазвернутьДеревоЗначенийСвойств(Элементы.ОтборУслуги, ОтборУслуги);
