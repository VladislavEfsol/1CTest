...
Процедура СоздатьУзелОбменаССайтом(ВхПараметры, АдресСайта, IDсайта, Пароль="") Экспорт
	...
	Если ВхПараметры.ЗаписьНаУслуги Тогда
		...
		СписокУслугСозданияСайта = ХранилищеСистемныхНастроек.Загрузить("СписокУслугСозданияСайта");
		Для каждого услуга Из СписокУслугСозданияСайта Цикл
			НовСтр = ПланОбменаСайт.Услуги.Добавить();
			НовСтр.Услуга = услуга.Значение;
			
			НовСтр.Длительность = 60;
		КонецЦикла;
		...
		Пока Выборка.Следующий() Цикл
			НовСтр = ПланОбменаСайт.Ресурсы.Добавить();
			НовСтр.Ресурс = Выборка.Ссылка;
		КонецЦикла;
		
		СохраненнаяТаблицаУслуг = ПланОбменаСайт.Услуги.Выгрузить();
		СохраненнаяТаблицаУслуг.Колонки.Добавить("Мастера", Новый ОписаниеТипов("СписокЗначений"));
		Для каждого стр Из СохраненнаяТаблицаУслуг Цикл
			стр.Мастера.Добавить(Неопределено, ОбменССайтом.НадписьВсеГруппы());
		КонецЦикла;
		ПланОбменаСайт.СохраненнаяТаблицаУслуг = Новый ХранилищеЗначения(СохраненнаяТаблицаУслуг);
	КонецЕсли;
