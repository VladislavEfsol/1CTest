...
Процедура ЗаполнитьДанныеСтатьиДДС()
	...
	|			И ВТ_ОборотыПоСтатьямВалюте.Статья В
	|					(ВЫБРАТЬ
	|						ВТ_ОборотыПоСтатьямВалюте.Статья
	|					ИЗ
	|						ВТ_ОборотыПоСтатьямВалюте КАК ВТ_ОборотыПоСтатьямВалюте
	|					СГРУППИРОВАТЬ ПО
	|						ВТ_ОборотыПоСтатьямВалюте.Статья
	|					ИМЕЮЩИЕ
	|						СУММА(ВТ_ОборотыПоСтатьямВалюте.СуммаПриход) <> СУММА(ВТ_ОборотыПоСтатьямВалюте.СуммаРасход))) КАК ВложенныйЗапрос
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_ДетальныеЗаписи КАК ВТ_ДетальныеЗаписи
	|			ПО ВложенныйЗапрос.Статья = ВТ_ДетальныеЗаписи.Статья
