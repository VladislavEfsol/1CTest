...
Функция ТекстЗапросаСерииОстатки()
	...
	|ПОМЕСТИТЬ втСерийныеНомераОстатки
	|ИЗ
	...
	|
	| //УПОРЯДОЧИТЬ ПО СерийныйНомер.Наименование
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	СерийныеНомераОстатки.СерийныйНомер КАК СерийныйНомер,
	|	СерийныеНомераОстатки.СерийныйНомер.Наименование КАК Наименование
	|ИЗ
	|	втСерийныеНомераОстатки КАК СерийныеНомераОстатки
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	СерийныеНомера.СерийныйНомер,
	|	СерийныеНомера.СерийныйНомер.Наименование КАК Наименование
	|ИЗ
...
Процедура ЗаполнитьСерийныеНомераПоНаличию(ДобавитьКоличество)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "//ПЕРВЫЕ N", " ПЕРВЫЕ "+Формат(ДобавитьКоличество,"ЧГ=0")+" ");
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "//УПОРЯДОЧИТЬ ПО СерийныйНомер.Наименование", " УПОРЯДОЧИТЬ ПО СерийныйНомер.Наименование ");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗаписьRFID_Серии" Тогда
		...
		СообщениеОбОшибке = "";
		RFIDКлиентСервер.ЗаполнитьФлагиРаботыСМеткой(ТекущиеДанные, GTIN, Неопределено, НастройкиИспользованияСерий, ЭтоМаркировкаПерсонифицированнымиКиЗ, СообщениеОбОшибке);
